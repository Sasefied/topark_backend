...
const confirmOperationSellOrderItem = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    console.log(`Current status for item ${id}: ${orderItem.status}`);

    // Prevent confirmation if status is already PALLETTE_READY
    if (orderItem.status === OrderItemStatusEnum.PALLETTE_READY) {
      throw new BadRequestError("Order item is already confirmed as Pallette Ready");
    }
    ...
    responseHandler(
      res,
      200,
      "Order item confirmed as Pallette Ready successfully",
      "success"
    );
  }
);
...
const markAsShippedOperationSellOrder = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (orderItems.some((item) => item.status !== OrderItemStatusEnum.PALLETTE_READY)) {
      throw new BadRequestError("All order items must be confirmed (PALLETTE_READY) to mark as shipped");
    }
